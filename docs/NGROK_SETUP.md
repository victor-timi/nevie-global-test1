...
4. **Use this URL in n8n HTTP Request node**:
   - URL: `https://YOUR-UNIQUE-ID.ngrok-free.app/nevie/test` (replace with your actual ngrok URL)
   - Method: POST
   - Body: `{"message": "={{ $json.body.message }}"}`

## Example ngrok URL

**API Endpoint Format**: `https://YOUR-UNIQUE-ID.ngrok-free.app/nevie/test`

**Test it** (replace with your actual ngrok URL):

```bash
curl -X POST "https://YOUR-UNIQUE-ID.ngrok-free.app/nevie/test" \
